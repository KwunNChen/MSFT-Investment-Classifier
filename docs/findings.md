...
* train accuracy was 60.4% vs 52.9% on test, a 7.5 point gap that looks like overfitting until you notice always_up drops from 60.4% to 52.9% between the same two eras. Exactly the same 7.5 points, with no learning involved at all
...
* train accuracy was 100.0%. It answered all 1,985 training days perfectly, then scored 50.1% on test: below always up, barely above a coin flip
    * Basically, the 49.9 point gap IS overfitting. Next to logistic regression's 7.5 point gap (which was just the flatter market) you can see the difference between memorization and a regime shift
* unlike logistic regression, the forest was NOT a rubber stamp. It said up on 351 days and down on 146, with probabilities swinging from 0.15 to 0.97, and those decisions went wrong often enough to cost it the baseline
...
* where that leaves v1: the simple model learned nothing and matched the baseline, the flexible model memorized everything and still fell short of it. There is no signal in these four clues to find
...
* given four standard technical clues, logistic regression concluded none of them beat "MSFT usually drifts up," and became the always up baseline with extra steps
    * Basically, the model always predicted up. ALWAYS, 100% of the time (497 out of 497 test days). This is because its up probability never crossed below 0.5
    * This is because the probability starts near 60% thanks to the training set (60% of training days were up), and the near zero weights never push it far from there. It lived between 0.53 and 0.74 for two straight years
* the four piles (confusion matrix): 263 right (the up days), 234 wrong (the down days), 0 and 0 in the "said down" piles, because it never said down once
...
* results: logistic regression went 0 wins, 5 ties, 2 losses against always up. Random forest went 0 wins, 0 ties, 7 losses
    * the ties are the same rubber stamp behavior in other eras: it said up every day and matched the baseline to the decimal
    * the losses came from windows where it did say down sometimes, and paid for it. In the 2019 window it gave up nearly 10 points second guessing a bull market
* the biggest lesson in the whole project: the bar itself swings from 47% to 68% depending on the era
    * Basically, an accuracy number means nothing on its own. 58.4% was the model's best score in that window and also its worst defeat, because always up scored 68.0% on those exact same days
...
```
  ...
  1   2491   53.6%   1 win, 0 ties, 6 losses   2 wins, 0 ties, 5 losses
  5   2487   58.8%   0 wins, 5 ties, 2 losses  0 wins, 0 ties, 7 losses
 21   2470   64.5%   3 wins, 2 ties, 2 losses  4 wins, 0 ties, 3 losses
 63   2386   73.1%   0 wins, 5 ties, 1 loss    1 win, 0 ties, 5 losses
```

* total: 11 wins in 54 matchups, and no horizon gave either model a winning record
* the bar CLIMBS with the horizon: MSFT rose in 53.6% of 1 day windows but 73.1% of quarter long windows
...
* the N = 21 wins look like luck, not signal
    * Basically, at N = 21 today's label and tomorrow's label describe almost the same 21 day stretch. A 250 day test window only holds about twelve independent outcomes, so per window scores swing wildly (25.6% to 76.8%), and a few wins scattered across unrelated eras is what luck produces
